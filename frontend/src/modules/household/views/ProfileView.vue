...
onMounted(async () => {
  ...
  try {
    const record = await pb
      .collection('members')
      .getOne<MemberRecord>(authStore.memberId, { expand: 'user_id,household_id' });
    member.value = record;
...
<template>
  ...
  <BottomSheet v-model:open="showLeaveSheet" title="Leave household">
    <p class="confirm-text">
      ...
      <strong>{{ householdStore.name ?? 'your household' }}</strong> immediately. Your expense
      history will be preserved. Once you leave, you'll need a new invitation to rejoin.
    </p>
        ...
        <div class="pref-card avatar-card">
          ...
          >
            ...
            <span
              ...
              :class="{ 'is-loading': avatarUploadStatus === 'loading' }"
              aria-hidden="true"
            >
              <i
                :class="avatarUploadStatus === 'loading' ? 'pi pi-spin pi-spinner' : 'pi pi-camera'"
              />
            </span>
          </button>
          <input
