...
const { mockGetOne, mockUpdate, mockToastAdd, mockLogout, mockRouterPush, mockAuthSave, mockSend } =
  vi.hoisted(() => ({
    mockGetOne: vi.fn(),
    mockUpdate: vi.fn(),
    mockToastAdd: vi.fn(),
...
const STUBS = {
  InputText: {
    template:
      '<input :id="id" :value="modelValue" :maxlength="maxlength" @input="$emit(\'update:modelValue\', $event.target.value)" />',
    props: ['modelValue', 'id', 'maxlength', 'placeholder'],
    emits: ['update:modelValue'],
  },
  Button: {
    template:
      '<button :type="type || \'button\'" :disabled="disabled || loading" @click="$emit(\'click\', $event)">{{ label }}</button>',
    ...
    it('shows error toast when avatar upload fails', async () => {
      ...
      expect(mockToastAdd).toHaveBeenCalledWith(
        expect.objectContaining({
          severity: 'error',
          summary: "Couldn't update picture — try again",
        })
      );
    });
  });
